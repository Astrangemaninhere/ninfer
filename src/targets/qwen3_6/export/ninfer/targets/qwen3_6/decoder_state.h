...
struct DecoderStateSpec {
    ...
    // E2M1 K / ISO3 V over the first-stage error). An enabled NVFP4 layer
    // doubles its plane count; other storages ignore the entry.
...
struct PagedKVCacheLayout {
    ...
    // Resolved per-layer residual flag (NVFP4 layers only).
    std::array<bool, 16> layer_residual{};
