...
template <typename Geometry, typename Metadata, bool E8 = false>
__launch_bounds__(256) __global__
    ...
    if constexpr (E8) {
        ...
        const __half ksh_e = __float2half_rn(k_abs > 0.0f ? k_abs / 127.0f : 0.0f);
        ...
        if (token == 0 && kv_head == 0 && group == 0 && lane == 1) {
            printf("[dbg] fill lane1 k0=%.4f c0=%d k1=%.4f c1=%d\n", k0, c0, k1, c1);
        }
        if (token == 0 && kv_head == 0 && group == 0 && lane == 0) {
            printf("[dbg] fill lane0 k0=%.4f c0=%d\n", k0, c0);
        }
        ...
        if ((lane & 1) == 0) {
            ...
            const std::int64_t kb0 =
                gqa_kv_i4_code_index<Geometry>(page, kv_head, d0 >> 1, page_off);
            cache_k[kb0] = gqa_kv_pack_i4(static_cast<std::int8_t>(c0),
                                          static_cast<std::int8_t>(c0n));
            const std::int64_t kb1 =
                gqa_kv_i4_code_index<Geometry>(page, kv_head, d1 >> 1, page_off);
            cache_k[kb1] = gqa_kv_pack_i4(static_cast<std::int8_t>(c1),
                                          static_cast<std::int8_t>(c1n));
            const std::int64_t vb0 =
                gqa_kv_i4_code_index<Geometry>(page, kv_head, d0 >> 1, page_off);
            cache_v[vb0] = gqa_kv_pack_i4(static_cast<std::int8_t>(vc0),
                                          static_cast<std::int8_t>(vc0n));
            const std::int64_t vb1 =
                gqa_kv_i4_code_index<Geometry>(page, kv_head, d1 >> 1, page_off);
            cache_v[vb1] = gqa_kv_pack_i4(static_cast<std::int8_t>(vc1),
                                          static_cast<std::int8_t>(vc1n));
            if (token == 0 && kv_head == 0 && group == 0) {
                const std::uint8_t p0 = cache_k[kb0];
                const int back0       = gqa_kv_unpack_i4(p0, 0);
                const int back1       = gqa_kv_unpack_i4(p0, 1);
                printf("[dbg] fill k0=%.4f c0=%d c0n=%d back=%d/%d byte=%d\n",
                       k0, c0, c0n, back0, back1, static_cast<int>(d0 >> 1));
            }
        }
        if (lane == 0) {
            const std::int64_t scale_off =
                gqa_kv_quant_scale_index<Geometry>(page, kv_head, group, page_off);
            scale_k[scale_off] = ksh_e;
            scale_v[scale_off] = vsh_e;
        }
        return;
    }
...
__launch_bounds__(256) __global__ void gqa_attention_prefill_fill_i8_page_kernel(
    ...
    if constexpr (E8) {
        ...
        if ((lane & 1) == 0) {
            ...
            const int vc1n = static_cast<int>(gqa_kv_quant_i4_code(v1n_nb, vinv_e));
            const std::int64_t kb0 =
                gqa_kv_i4_code_index<Geometry>(physical_page, kv_head, d0 >> 1, page_off);
            cache_k[kb0] = gqa_kv_pack_i4(static_cast<std::int8_t>(c0),
                                          static_cast<std::int8_t>(c0n));
            const std::int64_t kb1 =
                gqa_kv_i4_code_index<Geometry>(physical_page, kv_head, d1 >> 1, page_off);
            cache_k[kb1] = gqa_kv_pack_i4(static_cast<std::int8_t>(c1),
                                          static_cast<std::int8_t>(c1n));
            const std::int64_t vb0 =
                gqa_kv_i4_code_index<Geometry>(physical_page, kv_head, d0 >> 1, page_off);
...
__global__ __maxnreg__(120) void gqa_attention_prefill_i8_kernel(
    ...
    auto issue_kv_tile = [&](int tile_k0) {
        ...
        // Revision 2b cold staging: raw nibble slots decoded inline into the
        // kernel's native int8 codes + fp16 g64 scales (same adapter as the
        // decode kernel; QK/PV math unchanged). Cold tiles fill synchronously
        ...
        if (tile_cold && v_slot != nullptr) {
            for (int key_l = tid; key_l < Bc; key_l += kGqaPrefillI8Threads) {
                ...
                if (key > max_query_abs) {
                    ...
                    std::int8_t* kdst =
                        &k_i8[(key_l * DB16 + gqa_prefill_swz(key_l, 0)) * 2];
#pragma unroll
                    for (int d = 0; d < D; d += 2) {
                        gqa_prefill_i8_store_swz(k_i8, key_l, d, 0);
                        gqa_prefill_i8_store_swz(k_i8, key_l, d + 1, 0);
                    }
                    store_vec(&v_i8[key_l * D], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 16], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 32], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 48], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 64], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 80], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 96], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 112], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 128], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 144], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 160], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 176], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 192], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 208], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 224], make_int4(0, 0, 0, 0));
                    store_vec(&v_i8[key_l * D + 240], make_int4(0, 0, 0, 0));
                    (void)kdst;
                    continue;
                }
...
#pragma unroll 1
        for (int chunk = tid; chunk < Bc * (D / 16); chunk += kGqaPrefillI8Threads) {
            ...
            if (key <= max_query_abs) {
                if constexpr (E8) {
                    ...
                    const std::int64_t off =
                        gqa_kv_i4_code_index<Geometry>(table_entry, kv_head, d / 2, key_l);
                    cp_async<8>(kd, &cache_k[off]);
                    cp_async<8>(vd, &cache_v[off]);
                } else {
                    const std::int64_t off =
                        gqa_kv_quant_code_index<Geometry>(table_entry, kv_head, d, key_l);
    ...
    if constexpr (E8) {
        unpack_tile(0);
        if (threadIdx.x == 0) {
            printf("[dbg] attn k0=%d k1=%d k2=%d k3=%d k4=%d k5=%d k6=%d k7=%d\n",
                         k_i8[0], k_i8[1], k_i8[2], k_i8[3], k_i8[4], k_i8[5], k_i8[6], k_i8[7]);
            printf("[dbg] attn ks0=%f\n", static_cast<double>(__half2float(k_scale_s[0])));
        }
    }
...
#pragma unroll
            ...
            if constexpr (E8) {
                if (warp == 0 && gid == 0 && kb == 0 && lid == 0) {
                    printf("[dbg] score00=%.3f score10=%.3f score20=%.3f qs0=%.4f ks0=%.4f vs0=%.4f\n",
                           score[0][0], score[1][0], score[2][0],
                           static_cast<double>(q_scale_r0[0]),
                           static_cast<double>(__half2float(k_scale_s[0])),
                           static_cast<double>(__half2float(v_scale_s[0])));
                }
            }
