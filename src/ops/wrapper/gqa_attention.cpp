...
std::uint32_t validate_cache(const PagedKVLayerView& cache, std::int32_t kv_heads, const char* op) {
    const bool nvfp4 = cache.dtype == DType::NVFP4;
    const bool fp8   = cache.dtype == DType::FP8_E4M3FN;
    const bool iso3  = cache.dtype == DType::ISO3;
    const bool packed16 = nvfp4 || fp8 || iso3;
    if ((cache.dtype != DType::BF16 && cache.dtype != DType::I8 && !packed16) ||
    ...
    const DType code_dtype =
        cache.dtype == DType::I8
            ? DType::I8
            : (fp8 ? DType::FP8_E4M3FN : (packed16 ? DType::U8 : DType::BF16));
    ...
    const std::int32_t code_leading =
        nvfp4 ? kHeadDim / 2 : (iso3 ? kHeadDim / 2 : kHeadDim);
    ...
    if (packed16) {
        constexpr std::int32_t groups = kHeadDim / kNvfp4QuantGroup;
...
std::size_t gqa_attention_workspace_capacity_bytes(std::int32_t q_heads, DType cache_dtype,
    ...
    if ((cache_dtype != DType::BF16 && cache_dtype != DType::I8 && cache_dtype != DType::NVFP4 &&
         cache_dtype != DType::FP8_E4M3FN && cache_dtype != DType::ISO3) ||
        batch_size <= 0 ||
