...
void attention_e8_for(const Tensor& q, const Tensor& positions, float scale,
                      ...
                      cudaStream_t stream) {
    std::fprintf(stderr, "[dbg] e8 attention q=%d tokens=%d k=%p v=%p ks=%p vs=%p\n",
                 static_cast<int>(q.ne[1]), static_cast<int>(q.ne[2]),
                 cache.k_pages.data, cache.v_pages.data, cache.k_scale_pages.data,
                 cache.v_scale_pages.data);
...
void append_e8_for(const Tensor& k, const Tensor& v, const Tensor& positions,
                   ...
                   cudaStream_t stream) {
    std::fprintf(stderr, "[dbg] e8 fill k=%p v=%p ks=%p vs=%p tokens=%d\n",
                 cache.k_pages.data, cache.v_pages.data, cache.k_scale_pages.data,
                 cache.v_scale_pages.data, static_cast<int>(k.ne[2]));
    const Tensor& cache_k = cache.k_pages;
    const Tensor& cache_v = cache.v_pages;
    const Tensor& cache_k_scale = cache.k_scale_pages;
    const Tensor& cache_v_scale = cache.v_scale_pages;
