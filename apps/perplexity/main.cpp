...
[[noreturn]] void usage_error(std::string_view message) {
    throw std::invalid_argument(std::string(message) +
                                ...
                                "[--kv-dtype bf16|int8|fp8] [--kv-layer-storage SPEC] [--kv-residual-layers L[,L...]] [--output <directory>]");
}
...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (option == "--corpus") {
            ...
            const std::string list = std::string(value("--kv-residual-layers"));
            std::size_t begin = 0;
            while (begin <= list.size()) {
                const std::size_t end = list.find(',', begin);
                const std::string_view token(
                    list.data() + begin,
                    (end == std::string::npos ? list.size() : end) - begin);
                if (!token.empty()) {
                    const std::size_t layer = parse_integer<std::size_t>(
                        std::string(token), "kv-residual-layers");
                    if (layer >= out.kv_residual_layers.size()) {
                        usage_error("--kv-residual-layers layer out of range");
                    }
                    out.kv_residual_layers[layer] = true;
                }
                if (end == std::string::npos) { break; }
                begin = end + 1;
            }
            out.kv_residual_explicit = true;
...
int run(const Options& options) {
    ...
    engine_options.kv_residual_explicit  = options.kv_residual_explicit;
